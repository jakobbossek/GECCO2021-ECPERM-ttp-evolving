...
//        System.out.println("tour="+Arrays.toString(tour));
		...
		while(iteration<maxIterations){
			...
			if(leftSolution.objectiveScore>middleSolution.objectiveScore && rightSolution.objectiveScore>middleSolution.objectiveScore){//rare local mins near greater max
				...
				if(debug)System.out.println("BOTH LESS: \t L: "+leftPower+" ("+leftSolution.objectiveScore+","+leftSolution.finalWeight+")"+" \t M: "+middlePower+" ("+middleSolution.objectiveScore+","+middleSolution.finalWeight+")"+" \t R: "+rightPower+" ("+rightSolution.objectiveScore+","+rightSolution.finalWeight+")");								
			}
		...
		if(debug)System.out.print("FINAL POWER: "+middlePower+", \t");
                ...
                if(debug)middleSolution.print();
                if(debug)middleSolution.printFull();
                if(debug)middleSolution.printFullForCCode();
                if(debug)middleSolution.toString();
                if(debug)System.out.println("\n\n");
...
public static TTPSolution ppGreedyRT3inner(TTPInstance instance, long maxRunTime, int[] tour, double power, int itemsPerCity, double[] weights, double[] profits, double[] dToGo, int[] cityTourIndex, boolean debug) {
        ...
        // added bugfix on 170504
        if (newSolution.finalWeight>instance.capacityOfKnapsack) {
            newSolution = new TTPSolution(tour, new int[instance.numberOfItems]);
            newSolution.computationTime = duration;
            instance.evaluate(newSolution, false);
        }
